...
export const ExposureVisualizer: React.FC<ExposureVisualizerProps> = ({ inputs, result }) => {
  ...
  return (
    <div className="flex flex-col gap-5 relative">
      ...
      {shutterAngle > 190 && (
        <div className="mt-1 mb-2 p-4 rounded-xl border border-red-500/30 bg-red-500/10 flex flex-col gap-3">
          <div className="flex items-center gap-2 text-red-400 font-bold text-sm">
            ...
            【警告】広いシャッター角度による副作用リスク
          </div>
          <p className="text-xs text-red-200/90 leading-relaxed">
            シャッター角度を広く（明るく）すると、シャッターが閉じている時間（安全地帯）が極端に短くなり、<strong>LED更新の干渉（スキャンライン）を隠すことが物理的に困難・あるいは不可能</strong>になります。また、過剰な露出やブラーにより以下の映像ノイズが発生します。
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
            <div className="bg-gray-950/60 p-3 rounded-lg border border-red-500/20 flex flex-col items-center text-center gap-2 relative overflow-hidden">
              <div className="w-full h-10 bg-slate-800 rounded relative flex flex-col justify-evenly overflow-hidden shadow-inner border border-gray-700">
                <div className="w-full h-3 bg-red-500/40 animate-pulse border-y border-red-500/60"></div>
                <div className="w-full h-3 bg-red-500/40 animate-pulse border-y border-red-500/60" style={{ animationDelay: '0.3s' }}></div>
              </div>
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-gray-200">スキャンラインの映り込み</span>
                <span className="text-[9px] text-gray-400 mt-0.5">隠せる安全地帯が狭すぎる</span>
              </div>
            </div>
            
            <div className="bg-gray-950/60 p-3 rounded-lg border border-red-500/20 flex flex-col items-center text-center gap-2">
              <div className="w-full h-10 bg-slate-800 rounded relative flex items-center justify-center border border-gray-700 overflow-hidden">
                 <div className="w-16 h-2.5 rounded-full bg-white/40 absolute blur-[2px]"></div>
                 <div className="w-8 h-2.5 rounded-full bg-white/80 absolute blur-[1px]"></div>
              </div>
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-gray-200">過剰なモーションブラー</span>
                <span className="text-[9px] text-gray-400 mt-0.5">被写体がブレて輪郭が消失</span>
              </div>
            </div>
          </div>
        </div>
